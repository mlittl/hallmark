# Slop test — 70 gates + pre-emit self-critique
...
Run this **before** the gate list, not after. Score the planned output 1–5 on each axis. Anything **< 3 on any axis triggers a revision pass** before the gate sweep — don't bring known weakness into a seventy-gate review.
...
40a. Is any **heading or display type italic** (`font-style: italic` on `h1`–`h6`, a `.*__title`, `.hero__title`, a wordmark, a stat figure, a footer statement, or an `<em>`/`<i>` inside a heading)? If yes, fail. Italic headers — above all the single italicised emphasis-word inside an upright headline — are a top AI tell. Headers are roman; emphasis comes from weight, accent colour, or a drawn underline. Italic is allowed *only* as body-copy emphasis inside running paragraphs. (Applies to every theme; Studio / Garden / Sport — historically italic-display — are reworked to roman.)
...
53. **Hero centred-everything.** Are eyebrow, title, lede, AND CTA all stacked centred on the same vertical axis? Auto-fail. Pick at most two centred elements; break alignment for the others. Centred-narrow heroes are admissible *only* on editorial / atelier voice, and even then the eyebrow or CTA should sit off-axis (margin-aligned, right-flush, or numeral-anchored).
...
70. **Hero content overflows the first viewport.** On a standard laptop viewport — test at **1280×800** (13″), not just 1440×900 — can the hero's essential content (eyebrow, headline, lede, **and the primary CTA**, plus any hero visual's focal point) all be seen **without scrolling**? If not, fail. This is the other half of gate 7: gate 7 caps the hero's *box* (`min-height`); this caps its *content*. A hero can satisfy `min-height: 75dvh` and still overflow because the content is intrinsically too tall — the usual culprits are an **oversized display clamp** (the `clamp()` max is bigger than what fits), **loose line-height** (display at ~1.2 instead of 1.0–1.1), a **lede that runs 3+ lines**, or **bloated `padding-block`** / stacked empty whitespace. Right-size to the fold: pull the display clamp's max down until the headline fits 2–3 lines above the fold, set display line-height 1.0–1.1, hold the hero lede to ~2 lines (≤ ~60 ch), and trim hero padding so the block fits one screen. **Don't overcorrect:** the target is *intentional fit*, not a shrunken or cramped hero — a hero that already fits passes untouched, and this never means tiny type or stripped whitespace. Long-form / art-directed statements (a poem broadside, a scroll-poster) may legitimately run taller than one screen, but even then the **first screen must read as a complete, deliberate composition** — never a headline sliced in half by the fold. Verify on the rendered output at 1280×800.
